...
class EncodingDetector {
    ...
    static func readWithAutoEncoding(data: Data) throws -> String {
        ...
        if let string = String(data: data, encoding: detection.encoding) {
            AppLogger.debug("📝 EncodingDetector: 使用 \(detection.encodingName) 解碼成功 (信心度: \(String(format: "%.0f%%", detection.confidence * 100)))")
            return string
        }
        ...
        for (encoding, name) in fallbackEncodings {
            if let string = String(data: data, encoding: encoding) {
                AppLogger.debug("📝 EncodingDetector: 降級使用 \(name) 解碼成功")
                return string
            }
        }
