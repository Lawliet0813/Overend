...
class PDFService {
    ...
    /// 獲取應用程式的附件存儲目錄
    static func getAttachmentsDirectory() throws -> URL {
    ...
    /// - Returns: 新的檔案路徑
    static func copyPDFToStorage(from sourceURL: URL, for entryID: UUID) throws -> URL {
        // 檢查檔案大小
    ...
    /// - Returns: (頁數, 提取的文字)
    static func extractPDFMetadata(from url: URL) throws -> (pageCount: Int, extractedText: String?) {
    ...
    ///   - attachment: 要刪除的附件
    ///   - context: Core Data 上下文
    static func deleteAttachment(_ attachment: Attachment, context: NSManagedObjectContext) throws {
    ...
    static func selectPDFFile(completion: @escaping (URL?) -> Void) {
        #if canImport(AppKit)
