...
public class GeminiService: ObservableObject {
    ...
    private func loadAPIKey() {
        if let key = KeychainHelper.load(key: "gemini_api_key") {
            ...
            if key == "••••••••••••••••" {
                ...
                try? KeychainHelper.delete(key: "gemini_api_key")
            } else {
                self.apiKey = key
    ...
    private func saveAPIKey() {
        try? KeychainHelper.save(key: "gemini_api_key", value: apiKey)
    }
    ...
    func generateContent(prompt: String, systemInstruction: String? = nil) async throws -> String {
        ...
        guard let url = URL(string: "\(baseURL)/\(defaultModel):generateContent?key=\(apiKey)") else {
    ...
    func uploadFile(fileURL: URL, displayName: String? = nil) async throws -> GeminiFile {
        ...
        guard let startURL = URL(string: "https://generativelanguage.googleapis.com/upload/v1beta/files") else {
            throw GeminiError.apiError("Invalid Upload URL")
        }
        ...
        guard let requestURL = URL(string: uploadURL) else {
            throw GeminiError.apiError("Invalid Upload URL from Response")
        }
        var uploadRequest = URLRequest(url: requestURL)
    ...
    func extractPDFMetadata(from fileURL: URL) async throws -> ExtractedBibTeX {
        ...
        let prompt = """
        ## 任務
        分析這份學術 PDF 文件（檔名：\(filename)），提取完整的書目資訊。
        
        ## 分析步驟
        1. **識別文獻類型**: 根據內容判斷是期刊論文、會議論文、學位論文、書籍還是技術報告
        2. **判斷是否為掃描檔**: 檢查文字是否清晰可選。**若是掃描檔或文字難以識別，請直接使用檔名「\(filename)」作為標題（去除副檔名）。**
        3. **提取基本資訊**: 標題、作者、年份、DOI
        4. **提取出版資訊**: 期刊/會議名稱、卷號、期號、頁碼、出版社
        5. **提取學術資訊**: 摘要、關鍵詞、機構/學校
        
        ## 特別注意
        - 標題：找出主標題和副標題（如有），使用完整格式。**對於掃描文件或 OCR 識別困難的文件，檔名具有最高優先級。**
...
// MARK: - Note
// Using global KeychainHelper from Utilities/KeychainHelper.swift
