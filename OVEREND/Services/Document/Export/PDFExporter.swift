...
class PDFExporter: NSObject {
    ...
    private static var activeExporters: [PDFExporter] = []
    ...
    static func export(
        ...
        print("📄 PDF Export - HTML 長度：\(html.count) 字元")
        print("📄 PDF Export - 內容預覽：\(String(html.prefix(500)))")
        print("📄 PDF Export - NSAttributedString 長度：\(content.length) 字元")
        print("📄 PDF Export - 純文字預覽：\(String(content.string.prefix(200)))")
    ...
    static func exportFromHTML(
        ...
        print("📄 PDF Export (from HTML) - HTML 長度：\(html.count) 字元")
        print("📄 PDF Export (from HTML) - 內容預覽：\(String(html.prefix(500)))")

        loadHTMLAndExport(html, exporter: exporter)
    }

    /// 載入 HTML 並執行 PDF 匯出的共用方法
    private static func loadHTMLAndExport(_ html: String, exporter: PDFExporter) {
    ...
    private func createPDFWithWebKitAPI() {
        guard let webView = webView else {
            finishWithError(NSError(domain: "PDFExporter", code: -1, userInfo: [NSLocalizedDescriptionKey: "WebView 未初始化"]))
            return
        }
    ...
    private func cleanup() {
        webView = nil
        PDFExporter.offscreenWindow?.contentView = nil
        PDFExporter.activeExporters.removeAll { $0 === self }
    }
}
